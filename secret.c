...
/* Static Variables */
PRIVATE unsigned char buff[SECRET_SIZE]; /* store secret */
PRIVATE size_t message_len = EMPTY_STRING; /*length of secret */
PRIVATE uid_t owner = NO_OWNER; /*UID of secret owner */
    ...
    message *m;
{
    uid_t current;
    struct ucred creds;
    if(getnucred(m->IO_ENDPT, &creds) != 0){
        return EPERM;
    }
    current = creds.uid;

    if(owner == NO_OWNER) {
        if(m->COUNT & R_BIT == R_BIT){
            return OK;
        } else if (m->COUNT & W_BIT == W_BIT){
